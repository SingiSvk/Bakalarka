    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        this.roomName.setOnClickListener(v -> {
            ...
            if(!roomName.matches("")){
                ...
                new Thread(() -> new RoomDatabaseController().updateRoomInDB(room)).start();
        ...
        this.personInfo.setOnClickListener(v -> {
            ...
            if(!personName.matches("")){
                room.getPatient().setName(personName);
                this.personChange = true;
            }
            if(!personAge.matches("")){
                room.getPatient().setAge(Integer.parseInt(personAge));
                this.personChange = true;
            }
            if (personChange){
                new Thread(() -> new RoomDatabaseController().updatePersonInDB(roomId, room.getPatient())).start();
                wearController.sendPatientData(roomId, room.getPatient());
        ...
        this.newIllness.setOnClickListener(v -> {
            ...
            if(!illnessName.matches("")){
                ...
                room.getPatient().getIllnesses().add(illness);
