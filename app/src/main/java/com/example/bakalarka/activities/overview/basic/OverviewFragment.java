...
import com.example.bakalarka.activities.dialogs.DeleteRoomDialog;
...
public class OverviewFragment extends Fragment {
    ...
    TextView temperatureText, humidityText, pressureText, vocText, events;
    ...
    public void writeEvents(){
        ...
        for (Event event: eventController.getLast3Events(room.getEvents())){
            try {
                events.append(eventController.eventToString(event));
            } catch (ParseException e) {
                e.printStackTrace();
            }
            events.append("\n");
        }
        this.events.setText(Html.fromHtml(events.toString()));
    }
    ...
    private void setRoomSettings(){
        ...
        textView.setText(Html.fromHtml("<font color='black'>Meno: </font>" + this.room.getPatient().getName()));
        textView = this.view.findViewById(R.id.person_age);
        int age = this.room.getPatient().getAge();
        if (age > 0){
        ...
        if(this.room.getPatient().getIllnesses() != null){
            for (Illness illness: this.room.getPatient().getIllnesses()){
    ...
    private void findElements(){
        ...
        this.events = view.findViewById(R.id.events);
    }
