...
public class NotificationController {
    ...
    public void showNotification(@NonNull Notification notification, int risk, int roomId){
        if (risk == ConditionsController.HIGH){
            notification.setPlaySound(true);
            createBuilder(notification, NotificationCompat.PRIORITY_HIGH, roomId);
        }else if (risk == ConditionsController.LOW){
            notification.setPlaySound(false);
            createBuilder(notification, NotificationCompat.PRIORITY_DEFAULT, roomId);
        }
    ...
    private void createBuilder(@NonNull Notification notification, int priority, int roomId){
        Intent intent = new Intent(MainActivity.context, OverviewAllRoomsActivity.class);
        intent.putExtra("page", roomId);
        PendingIntent pendingIntent = PendingIntent.getActivity(MainActivity.context, roomId, intent, 0);

        NotificationCompat.Builder builder = new NotificationCompat.Builder(MainActivity.context, "channel"+roomId)
